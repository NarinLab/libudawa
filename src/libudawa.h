...
void recordLog(uint8_t level, const char* fileName, int lineNumber, const char* functionName)
{
  ...
  sprintf_P(formattedLog, PSTR("[%s][%d][%s:%d] %s: %s"), levels, ESP.getFreeHeap(), lineNumber, functionName, logBuff);
  if(tb.connected())
  {
    StaticJsonDocument<DOCSIZE> doc;
  ...
  else
  {
    ...
    sprintf_P(_logRec[_logRecIndex], PSTR("[%s][%d][%s:%d] %s: %s"), levels, ESP.getFreeHeap(), fileName, lineNumber, functionName, logBuff);
    _logRecIndex++;
  }
